...
export default async function AdminFormResponsesPage({ params, searchParams }: FormResponsesPageProps) {
  ...
  function makePageHref(targetPage: number): string {
    ...
    return `/admin/responses/${formId}?${params.toString()}`;
  }
  ...
  return (
    <section className="space-y-6">
      ...
      {submissions.length > 0 ? (
        <ResponsesPageTabs submissions={submissions} stats={questionAnalytics} distribution={ratingDistribution} currentPage={currentPage} totalPages={totalPages} prevPageHref={prevPageHref} nextPageHref={nextPageHref} tallyRows={tallyRows} formId={formId} formTitle={form.title} personnelName={adminName} />
      ) : (
