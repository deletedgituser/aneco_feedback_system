...
export default async function FormDetailPage({ params, searchParams }: PageProps) {
  ...
  async function addQuestionAction(formData: FormData) {
    ...
    if (label.length > 191) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Question+text+must+be+191+characters+or+fewer.`);
    }

    if (categoryPart.length > 120) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Category+must+be+120+characters+or+fewer.`);
    }
  ...
  async function updateQuestionAction(formData: FormData) {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        redirect(`/forms/${parsedId}?toastType=error&toastMessage=Unable+to+update+question+with+provided+values.`);
      }
      throw error;
    }
    ...
    revalidatePath(`/forms/${parsedId}`);
    revalidatePath(`/kiosk/forms/${parsedId}`);
    redirect(`/forms/${parsedId}?toastType=success&toastMessage=Question+updated.`);
  }
  ...
  async function deleteQuestionAction(formData: FormData) {
    ...
    if (question.isOverallSatisfaction) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Overall+Satisfaction+question+cannot+be+deleted.`);
    }
  ...
  async function moveQuestionAction(formData: FormData) {
    ...
    if (!Number.isInteger(questionId) || questionId <= 0 || !["up", "down"].includes(direction)) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Invalid+request.`);
    }
    ...
    if (question.isOverallSatisfaction) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Overall+Satisfaction+question+cannot+be+reordered.`);
    }
    ...
    if (direction === "up" && currentIndex === 0) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Cannot+move+question+up+further.`);
    }

    if (direction === "down" && currentIndex === nonOverallQuestions.length - 1) {
      redirect(`/forms/${parsedId}?toastType=error&toastMessage=Cannot+move+question+down+further.`);
    }
    ...
    revalidatePath(`/forms/${parsedId}`);
    revalidatePath(`/kiosk/forms/${parsedId}`);
    redirect(`/forms/${parsedId}?toastType=success&toastMessage=Question+reordered.`);
  }
